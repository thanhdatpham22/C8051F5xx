...
void main (void)
{
   ...
   U8 test_byte; 
   U8 i;                      // Used as a temporary variable
   ...
   SFRPAGE = ACTIVE_PAGE;              // Set for printf()
    ...
    while (1)                          // Vòng l?p vô h?n
    {
        ...
        LED = !LED;                    // Ð?o tr?ng thái LED
        Delay_ms(1000);                // Ch? 1 giây
    }
   // Fill EEPROM with 0xFF's
   /*LED = 1;
   SEND_String(&test_byte, 1);
   printf("Filling with 0xFF's...\n");
   for (address = 0x0000; address <= 0x00FF; address += 8)
   {
      for(i =0 ; i < 8; i++)
	  {
	  	test_byte = i +1;
		EEPROM_Write(address + i, test_byte);
		if ((address % 16) == 0 && i == 0)
         {
...
void PORT_Init (void)
{
   ...
   SFRPAGE = CONFIG_PAGE;
	...
	P0MDOUT   = 0x34; //spi P0.2 clk , P0.3 MISO(Opendrain), P0.4 MOSI (Pull), P0.5 EN
	P0SKIP  = 0xC3;  

    P1MDOUT = 0x08; //UART1 P1.3 TX push pull     
	P1SKIP = 0x07;						

	P2MDOUT |= 0x03;  //LED P2.0, P2.1                  
    P2SKIP  |= 0x03
	;
	XBR0     = 0x00; //Enable crossbar
	XBR1 = 0x04;
    XBR2    = 0x42;  
